    ...
    def get_data_loop():
        ...
        if os.path.isfile('lock.txt'):
            ...
            with open('envfile.data', 'rb') as filehandle:  
                ...
                distance = pickle.load(filehandle)
            ...
            label_dict['sonar_val'].config(text=str(distance))
...
sonar_txt = Label(frame1, font=("Calibri", 12), bd=3,  text="Distance in.")
sonar_txt.place(relx=0.1, rely=0.5, relwidth=0.15, relheight=0.06, anchor='n')
sonar_val = Label(frame1, font=("Calibri", 12), width=8, relief=SUNKEN, fg="green")
sonar_val.place(relx=0.1, rely=0.55, relwidth=0.15, relheight=0.06, anchor='n')
label_dict.update({'sonar_val' : sonar_val})
